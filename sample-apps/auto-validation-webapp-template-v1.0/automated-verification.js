            ...
            exec('curl -X POST localhost:3001/api/generate-test-log', (error, stdout, stderr) => {
                if (error) {
                    ...
                    try {
                        ...
                        if (response.success === true && response.fileName && response.filePath) {
                            this.results.details.step2 = `✅ API実行成功: ${response.fileName}`;
                            this.results.step2 = true;
                            this.results.generatedFile = response.filePath;
                            resolve(true);
                        } else {
